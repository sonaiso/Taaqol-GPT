...
def _schema_allowed_verdicts() -> set[str]:
    payload = json.loads(_COVERAGE_SCHEMA.read_text(encoding="utf-8"))
    return set(payload["properties"]["expected_verdict"]["enum"])


def _assert_case_allowed_by_schema(case: dict[str, object]) -> None:
    schema = json.loads(_COVERAGE_SCHEMA.read_text(encoding="utf-8"))
...
def test_bridgeproof_maps_to_bridge_required_and_bridge_family() -> None:
    row = next(item for item in _read_csv_rows() if item["proof_object"] == "BridgeProof")
...
def test_identityproof_not_residual_only() -> None:
    row = next(item for item in _read_csv_rows() if item["proof_object"] == "IdentityProof")
...
def test_evidenceproof_cannot_promote_rank() -> None:
    row = next(item for item in _read_csv_rows() if item["proof_object"] == "EvidenceProof")
    assert row["rank_promotion_allowed"] == "false"


def test_coverageproof_cannot_emit_computed_verdict() -> None:
    row = next(item for item in _read_csv_rows() if item["proof_object"] == "CoverageProof")
...
def test_forbidden_runtime_files_remain_absent() -> None:
    assert not (_REPO_ROOT / "binding_kernel.py").exists()
    assert not (_REPO_ROOT / "coverage_matrix_v0.1.yaml").exists()
