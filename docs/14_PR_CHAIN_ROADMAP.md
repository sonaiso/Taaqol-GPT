...
USM-C3.1 Capability Provenance and Trace Hardening                       ✓ done
...
GPT-R5
    Origin   : docs/54 (operational objective) + docs/55 (origin boundary)
               + GPT-R4 readiness.
    Output   : OriginBindingClaim, OriginBindingGateResult,
               GPT_ORIGIN_BINDING_TRANSITION_CONTRACT, and helpers that bind
               MantuqGPT / MafhumGPT claim surfaces to required Knowledge
               Origins with visible OriginResiduals.
    Forbidden: ReasonablenessGate, EvidenceSupportGate,
               ReasonablenessVerdict, GPTAnswerReasonablenessVerdict,
               ReasonablenessPipeline, audit integration, hidden residuals,
               claim → reasonableness shortcuts, and truth/certificate
               language.
...
LAFZI-B3
    Origin   : docs/59 (Lafzi Madlul Correspondence Law) + completed LAFZI-B2,
               as the next separately staged implementation PR.
    Output   : SourceIdentityGate only: bounded source-identity candidate
               verdict surface with visible residuals, rank ceiling, and trace
               discipline.
    Forbidden: bundling steps, hidden lafzi residuals, Wad'iMadlulClosed,
               mutabaqah, tadammun, iltizam, relation, composition, ifādah,
               hukm, tanzīl, reality, adapter/audit mutation, and global
               FailureCode expansion unless a later law explicitly promotes a
               local residual.
    Law      : LAFZI-B3 closes only source-identity candidacy and does not
               execute FormState/InternalWordPath/LafziResidualAudit or
               LafziMadlulClosed.

LAFZI-B4
    Origin   : docs/59 (Lafzi Madlul Correspondence Law) + completed LAFZI-B3,
               as the next separately staged implementation PR.
    Output   : FormStateGate only: bounded form-state candidate verdict
               surface with visible residuals, rank ceiling, and trace
               discipline.
    Forbidden: bundling steps, hidden lafzi residuals, Wad'iMadlulClosed,
               mutabaqah, tadammun, iltizam, relation, composition, ifādah,
               hukm, tanzīl, reality, adapter/audit mutation, and global
               FailureCode expansion unless a later law explicitly promotes a
               local residual.
    Law      : LAFZI-B4 closes only form-state candidacy and does not execute
               InternalWordPath/LafziResidualAudit or LafziMadlulClosed.

LAFZI-B5
    Origin   : docs/59 (Lafzi Madlul Correspondence Law) + completed LAFZI-B4,
               as the next separately staged implementation PR.
    Output   : InternalWordPathGate only: bounded internal-word-path
               candidate verdict surface with visible residuals, rank
               ceiling, and trace discipline.
    Forbidden: bundling steps, hidden lafzi residuals, Wad'iMadlulClosed,
               mutabaqah, tadammun, iltizam, relation, composition, ifādah,
               hukm, tanzīl, reality, adapter/audit mutation, and global
               FailureCode expansion unless a later law explicitly promotes a
               local residual.
    Law      : LAFZI-B5 closes only internal-word-path candidacy and does not
               execute LafziResidualAudit or LafziMadlulClosed.

LAFZI-B6 through LAFZI-B7
    Origin   : docs/59 (Lafzi Madlul Correspondence Law) + completed LAFZI-B5,
               each as a separately staged implementation PR.
    Output   : the remaining lafzi gate sequence: LafziResidualAudit; and
...
LAFZI-D2
    ...
    Status   : completed runtime MutabaqahGate bounded to D2 only.
    Output   : MutabaqahGateResult preserving the D1 madlūl boundary,
               included/excluded surface, domain, scope, residuals, rank, and
               trace.
    Forbidden: TadammunGate, IltizamGate, DalalahMatrix closure, ifādah,
    ...
    Law      : D2 receives CoupledDalalahSurface and proves only bounded
...
LAFZI-D5
    Origin   : docs/62 (Coupled Dalālah Matrix Law) + completed LAFZI-D4,
               IltizamGateResult.
    Status   : completed runtime DalalahMatrixResidualAudit bounded to D5
               only.
    Output   : DalalahMatrixResidualAuditResult preserving D4 residual
               ancestry, madlūl boundary, included/excluded surface, domain,
               scope, rank, trace, and D1-D4 predecessor refs.
    Forbidden: DalalahMatrix closure, WordCapability, ifādah, mafhūm, hukm,
               tanzīl, reality, truth value, ontology, adapter/audit mutation,
               hidden dalālah residuals, rank promotion, downstream output,
               residual erasure, and global FailureCode expansion.
    Law      : D5 receives IltizamGateResult and audits residual visibility
               only; it does not resolve residuals, close the matrix, open
               word capability, or produce final meaning.
...
GPT-R6
    ...
    Output   : GPT_REASONABLENESS_GATES_TRANSITION_CONTRACT,
               ReasonablenessGateDecision, and ReasonablenessGateReport
               over six bounded checks:
               maqam fit, origin binding completeness, evidence support,
               contradiction, forbidden leap, and rank/residual policy.
    Forbidden: final reasonableness verdict, verdict-state labeling,
               pipeline orchestration, audit integration, hidden residuals,
               rank promotion without evidence, output without trace, and
               certificate/authority language.
...
DAL-A7 runtime gates (usage / loan / unvocalized / deletion residual gates only)
    ...
    Status   : ✓ done; DAL-A7 runtime gates are active only as bounded
    ...
    DAL_A7_RUNTIME_VERDICT = {
      ...
      lafzi_b_status: DEFERRED,
      semantic_hukm_reality_status: FORBIDDEN,
      next_permitted_pr: DAL-A7.1 corrective hardening only,
      parser_runtime: NOT_OPENED,
      morphology_runtime: NOT_OPENED,
      syntax_runtime: NOT_OPENED,
      semantic_runtime: NOT_OPENED,
...
DAL-A7.1 harden DAL-A7 LAFZI/LAFZI-B handoff deferral semantics (corrective only)
    ...
    Output   : DAL-A7 downstream handoff distinction only:
               ...
               requests remain forbidden.
    Forbidden: DAL-A8 integration opening, DalAloneClosed -> LafziMadlulGate
               implementation, LAFZI-B0..B7 runtime opening, parser/morphology/
               syntax runtime opening, and semantic/ifadah/mafhum/hukm/truth/
               certainty/reality outputs.
    Law      : deferred future-boundary acknowledgement must not be mislabeled
               as a direct forbidden straight-line output; rank promotion
               remains forbidden and trace visibility remains mandatory.

    DAL_A7_1_RUNTIME_VERDICT = {
      status: DEFERRED_OR_REFUSED_BY_HANDOFF_KIND,
      scope: DAL_A7_DOWNSTREAM_HANDOFF_DEFERRAL_HARDENING_ONLY,
      upstream_required: DAL_A6_RUNTIME_CLOSED,
...
Amendment-20 (PV-A4.1 — Maʿqūl Branch Discipline Law)
    Branch   : law-only clarification covenant (Family A).
    Chosen   : PV-A4.1 revises docs/51 from "Maʿqūl Interlock Law" (which
               proposed Maʿqūl al-Manṭūq and Maʿqūl al-Mafhūm as new
               independent runtime layers) to "Maʿqūl Branch Discipline
               Law" — a clarification covenant recognising that the existing
               dalālah chain (PR-15 through PV-A4) already embodies the
               rational ordering that "maʿqūl" names.
    Rationale: The prior proposal (MaqulMantuqClosureCandidate,
               MaqulMafhumClosureCandidate, prove_maqul_*_closure())
               duplicated what MufradDalālahClosure + RelationClosure +
               IfādahCandidate + MantuqClosure already prove. ManṭūqClosure
               is not "raw" — it consumes IfadahVerdict (which requires
               RelationClosure which requires MufradDalālahClosure) plus
               MaqamContextBoundaryVerdict plus relation_closure_ref
               continuity. Adding a new runtime layer on top of this was
               architectural hallucination (هلوسة معمارية):
               "لا تعالج التفكك بمزيد من التفكيك."
    Effect   : after PV-A4.1 merges:
                 * docs/51 is ratified as a clarification covenant;
                 * the 12-stage dalālah sequence is named Maʿqūl al-Dalālah;
                 * the discipline rules (no mafhūm without full sequence,
                   no hukm without branch discipline, iltizām is condition
                   not necessitator, haqīqah before majāz) are binding;
                 * PV-A4.2, PV-A4.3, PV-A5, PV-A6 are withdrawn;
                 * MaqulMantuqClosureCandidate, MaqulMafhumClosureCandidate,
                   and all ten reserved failure codes are withdrawn;
                 * MAFHUM_CONSUMES_RAW_MANTUQ_BEFORE_MAQUL is discharged
                   (MantuqClosureVerdict is constitutionally complete);
                 * future branches (haqīqah/majāz, naql, lexical relations)
                   follow docs/47 admission rules directly;
                 * no new carrier, enum, or operation is introduced.
    Deferred : all items deferred by Amendment-1 through Amendment-19
               remain deferred. General Energy Conservation Law (PV-E0)
               remains deferred. Future dalālah branches (haqīqah/majāz,
               naql, lexical relations) to be numbered by future amendments.
    Forbidden: this amendment ships no runtime code, no src/ changes,
               no new carriers, no new enums, no new operations, no
               adapter or audit change, no new runtime dependency,
...
Amendment-73 (USM-C3 — Bounded capability evaluator runtime)
    ...
    Sequencing:
               USM-C3.1 is complete; USM-C4 is the current bounded runtime
...
Amendment-75 (LEXICON-L1 + LEXICON-SLOT-L0 — Constitutional registration)
    Branch   : constitutional lexicon branch registration after USM-C5.
    Chosen   : register:
               (a) LEXICON-L1 as ✓ done (docs/99 law-only constitutional
                   lexicon licensing architecture boundary);
               (b) LEXICON-SLOT-L0 as current (docs/100 boundary plus
                   bounded lexical slot/runtime contract surface).
    Effect   : updates docs/14 chain table and amendment record to close
               the governance-position gap by making docs/99 and docs/100
               explicit chain-truth entries.
    Preserves: no semantic/hukm/truth/reality closure opening; no bypass
               from LexiconEntry/LexicalSlot to final meaning; residual
               visibility and trace obligations remain mandatory.
    Forbidden: treating lexical capability as commitment, claiming
               final contextual meaning from lexicon boundary outputs, or
               conflating law-only boundary registration with unrestricted
               execution licensing.
    Sequencing:
               LEXICON-SLOT-L0 is current until its dedicated successor
               step is admitted by a separate amendment.
    Trace    : docs/99 → docs/100 → src/taaqqul_slot_geometry/weight/
               lexicon_slot_geometry.py
